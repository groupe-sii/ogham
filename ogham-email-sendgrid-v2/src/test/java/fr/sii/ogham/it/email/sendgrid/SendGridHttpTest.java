...
public class SendGridHttpTest {
	...
	public void emailWithAttachments() throws MessagingException, JsonParseException, JsonMappingException, IOException {
		...
		server.verify(postRequestedFor(urlEqualTo("/api/mail.send.json"))
			...
			.withAnyRequestBodyPart(aMultipart("subject").withBody(equalTo(SUBJECT)))
			.withAnyRequestBodyPart(aMultipart("text").withBody(equalTo(CONTENT_TEXT)))
			.withAnyRequestBodyPart(aMultipart("files[04-Java-OOP-Basics.pdf]").withBody(equalTo(loadFile("/attachment/04-Java-OOP-Basics.pdf"))))
			.withAnyRequestBodyPart(aMultipart("files[ogham-grey-900x900.png]").withBody(equalTo(loadFile("/attachment/ogham-grey-900x900.png")))));
		// @formatter:on
	}
