...
[.fragment]
==== Test received emails

[.fragment]
--

[source, java, role="irrelevant-lines:4-5,7,19"]
----
@Rule
public GreenMailRule greenMail = new RandomPortGreenMailRule();  // <1>

@Test
public void userShouldReceiveConfirmationEmail() {
	// test your business code here that sends an email
	yourAccountService.newAccount(someAccount);
	
	// ensure that email is received with right information
	assertThat(greenMail).receivedMessages()                     // <2>
		.count(is(1))
		.message(0)
			.subject(is("Welcome !"))
			.to()
			...
			.body()
				.contentAsString(containsString("Confirm you email address by clicking"))
				.contentType(startsWith("text/html"));
}
----
<1> Start a SMTP server on a random port
<2> Make assertions on received emails using a simple API
...
[source, java, role="irrelevant-lines:4-5,7,16"]
----
@Rule
public SmppServerRule<SubmitSm> smppServer = new JsmppServerRule();  // <1>
...
public void userShouldReceiveConfirmationCode() {
	...
	assertThat(smppServer).receivedMessages()                        // <2>
		.count(is(1))
		.message(0)
			.content(containsString("Confirmation code: 1234"))
			.to()
				.number(is(someAccount.getPhoneNumber()));
}
----
<1> Start a SMPP server on a random port
<2> Make assertions on received SMS using a simple API
