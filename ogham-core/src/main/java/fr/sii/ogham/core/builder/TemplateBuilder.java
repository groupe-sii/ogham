 ...
 * {@link ThymeleafTemplateParserBuilder})</li>
 * </ul>
 * 
...
public class TemplateBuilder implements TemplateParserBuilder {
	...
	/**
	 * Enable FreeMarker template engine. This engine is used only if the
	 * associated detector ({@link FreeMarkerTemplateDetector}) indicates that
	 * FreeMarker is able to handle the provided template.
	 * 
	 * @return this builder instance for fluent use
	 */
	...
	public TemplateParser build() throws BuildException {
		...
		for (TemplateParserBuilder templateParserBuilder : detectors.values()) {
			...
			// TODO manage extra configuration for each {@link
	...
	private String resolve(String which, String value, String key, String defaultKey) {
		...
		if (resolved == null) {
			if (propertyResolver == null) {
				...
				resolved = propertyResolver.getProperty(key, propertyResolver.getProperty(defaultKey, ""));
