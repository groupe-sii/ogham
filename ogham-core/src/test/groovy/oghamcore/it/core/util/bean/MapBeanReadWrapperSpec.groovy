...
class MapBeanReadWrapperSpec extends Specification {
	...
	def "invalid map key (#crewName)"() {
		...
		where:
			...
			0         | null     || IllegalArgumentException | "key must not be null"
			...
			1         | null     || IllegalArgumentException | "key must not be null"
			...
			2         | null     || IllegalArgumentException | "key must not be null"
	...
	def "use size() on empty wrapped bean"() {
		when:
			def map = new MapBeanReadWrapper([])
		then:
			map.size()==0
			map.isEmpty()==true
	}
