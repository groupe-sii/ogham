...
import static fr.sii.ogham.email.JavaxMailConstants.DEFAULT_JAVAX_MAIL_IMPLEMENTATION_PRIORITY;
...
import fr.sii.ogham.email.sender.impl.javaxmail.JavaMailAttachmentHandler;
import fr.sii.ogham.email.sender.impl.javaxmail.JavaMailContentHandler;
import fr.sii.ogham.email.sender.impl.javaxmail.JavaMailInterceptor;
 ...
 */
@Priority(properties = "${ogham.email.implementation-priority.javamail}", defaultValue = DEFAULT_JAVAX_MAIL_IMPLEMENTATION_PRIORITY)
public class JavaxMailSender extends AbstractSpecializedSender<Email> {
	...
	public JavaxMailSender(PropertyResolver propertyResolver, JavaMailContentHandler contentHandler, JavaMailAttachmentHandler attachmentHandler, Authenticator authenticator) {
	...
	public JavaxMailSender(Properties properties, JavaMailContentHandler contentHandler, JavaMailAttachmentHandler attachmentHandler, Authenticator authenticator) {
		this(properties, contentHandler, attachmentHandler, authenticator, null);
	}

	public JavaxMailSender(Properties properties, JavaMailContentHandler contentHandler, JavaMailAttachmentHandler attachmentHandler, Authenticator authenticator, JavaMailInterceptor interceptor) {
		super();
	...
	public void send(Email email) throws MessageException {
		try {
			LOG.debug("Initialize Javax mail session with authenticator {} and properties {}", authenticator, properties);
			...
			LOG.info("Sending email using Javax Mail API through server {}:{}...", properties.getProperty("mail.smtp.host", properties.getProperty("mail.host")),
			...
			throw new MessageException("failed to send message using Javax Mail API", email, e);
		}
	}
	...
	private static boolean hasEmbeddableAttachments(Email email) {
		return email.getAttachments().stream().anyMatch(JavaxMailSender::isEmbeddableAttachment);
	}

	private static boolean hasDownloadableAttachments(Email email) {
		return email.getAttachments().stream().anyMatch(JavaxMailSender::isDownloadableAttachment);
	}
