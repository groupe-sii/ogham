...
@LogTestInformation
public class OghamSpringBoot3AutoConfigurationTests {
	...
	public void oghamAlone() throws Exception {
		contextRunner = contextRunner.withConfiguration(of(OghamSpringBoot3AutoConfiguration.class));
	...
	public void oghamInWebContext() throws Exception {
		contextRunner = contextRunner.withConfiguration(of(WebMvcAutoConfiguration.class, OghamSpringBoot3AutoConfiguration.class));
		contextRunner.run((context) -> {
			MessagingService messagingService = context.getBean(MessagingService.class);
			checkEmail(messagingService);
			checkSms(messagingService);
