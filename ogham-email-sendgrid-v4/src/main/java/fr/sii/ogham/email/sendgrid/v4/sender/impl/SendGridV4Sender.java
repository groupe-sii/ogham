...
public final class SendGridV4Sender extends AbstractSpecializedSender<Email> implements SendGridSender {
	...
	private void addAttachment(final Mail sendGridMail, final Attachment attachment) throws AttachmentReadException {
		try {
			...
			// SendGrid only supports ISO_8859_1 for attachments
			// see: https://github.com/anymail/django-anymail/issues/150
			String content = IOUtils.toString(attachment.getResource().getInputStream(), ISO_8859_1);
			sendGridAttachment.setContent(Base64Utils.encodeToString(content.getBytes(ISO_8859_1)));
			...
			sendGridAttachment.setType(mimetypeProvider.detect(content).toString());
